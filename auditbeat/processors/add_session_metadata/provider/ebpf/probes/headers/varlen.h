...
struct {
    ...
    __uint(max_entries, 1); // Will be resized by userspace to $(nproc)
...
struct varlen_field *vl_field__add(struct varlen_fields_start *fields, enum varlen_field_type type)
{
    struct varlen_field *new_field =
        (struct varlen_field *) (&fields->data[fields->size & EVENT_BUFFER_SIZE_HALF_MASK]);
    new_field->type = type;
    fields->nfields++;
    return new_field;
}

void vl_fields__init(struct varlen_fields_start *fields)
{
    fields->nfields = 0;
    fields->size = 0;
}

void vl_field__set_size(struct varlen_fields_start *vl_fields,
                        struct varlen_field *field,
                        size_t size)
{
    vl_fields->size += size + sizeof(struct varlen_field);
    field->size = size;
}
