...
int BPF_PROG(raw_tp__sched_process_fork,
             ...
             const struct task_struct *child)
{
    ...
    struct process_fork_event *event = get_event_buffer();
    if (!event)
        goto out;

    event->hdr.type = EVENT_PROCESS_FORK;
    event->hdr.ts = bpf_ktime_get_ns();
    event->hdr.hook_point = TRACEPOINT__SCHED_PROCESS_FORK;

    pid_info__fill(&event->parent_pids, parent);
    pid_info__fill(&event->child_pids, child);

    cred_info__fill(&event->creds, parent);
...
int BPF_PROG(raw_tp__sched_process_exec,
             ...
             const struct linux_binprm *binprm)
{
    ...
    struct process_exec_event *event = get_event_buffer();
    if (!event)
        goto out;

    event->hdr.type = EVENT_PROCESS_EXEC;
    event->hdr.ts = bpf_ktime_get_ns();
    event->hdr.hook_point = TRACEPOINT__SCHED_PROCESS_EXEC;

    pid_info__fill(&event->pids, task);
    cred_info__fill(&event->creds, task);
    ctty__fill(&event->ctty, task);
    ...
    // argv
    field = vl_field__add(&event->vl_fields, VL_FIELD_ARGV);
    size = argv__fill(field->data, ARGV_MAX, task);
    vl_field__set_size(&event->vl_fields, field, size);

    // env
    field = vl_field__add(&event->vl_fields, VL_FIELD_ENV);
    size = env__fill(field->data, ENV_MAX, task);
    vl_field__set_size(&event->vl_fields, field, size);

    // cwd
    field = vl_field__add(&event->vl_fields, VL_FIELD_CWD);
    size = resolve_path_to_string(field->data, &task->fs->pwd);
    vl_field__set_size(&event->vl_fields, field, size);

    // filename
    field = vl_field__add(&event->vl_fields, VL_FIELD_FILENAME);
    size = resolve_path_to_string(field->data, &binprm->file->f_path);
    vl_field__set_size(&event->vl_fields, field, size);
...
int BPF_KPROBE(kprobe__taskstats_exit, const struct task_struct *task, int group_dead)
{
    ...
    struct process_exit_event *event = get_event_buffer();
    if (!event)
        goto out;

    event->hdr.type = EVENT_PROCESS_EXIT;
    event->hdr.ts = bpf_ktime_get_ns();
    event->hdr.hook_point = KPROBE__TASKSTATS_EXIT;
...
int tracepoint__syscalls_sys_exit_setsid(struct trace_event_raw_sys_exit *args)
{
    ...
    struct process_setsid_event *event = get_event_buffer();
    if (!event)
        goto out;

    event->hdr.type = EVENT_PROCESS_SETSID;
    event->hdr.ts = bpf_ktime_get_ns();
    event->hdr.hook_point = TRACEPOINT__SYSCALLS_SYS_EXIT_SETSID;
